...
app_name = 'places'
urlpatterns = [
    path('<int:pk>/', cache_page(PAGE_CACHE_DURATION)(views.DetailView.as_view()), name='detail'),
    ...
    re_path(r'^map/tiles/(?P<z>\d+)/(?P<x>\d+)/(?P<y>\d+)/$', cache_page(PAGE_CACHE_DURATION)(views.MapTilesView.as_view()), name='map-tiles'),
]